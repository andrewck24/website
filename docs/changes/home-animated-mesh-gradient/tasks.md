...
- [ ] Add `--alt-mesh-bg: oklch(0.985 0 0)` to `:root` and `--alt-mesh-bg: oklch(0.051 0 0)` to `.dark` in `src/app/globals.css`. Verify: the CSS var resolves correctly in browser DevTools for both themes.
- [ ] Run `pnpm type-check`. Fix any errors before proceeding.
...
- [ ] Create `src/components/home/mesh-gradient.tsx`. The component exports `MeshGradientBackground()` — no props, no `"use client"` directive.
- [ ] Inside the component, render an `<svg>` with `className` that applies `position:absolute; inset:0; width:100%; height:100%` and `preserveAspectRatio="xMidYMid slice"` and `viewBox="0 0 1400 600"`.
- [ ] Add `<rect width="1400" height="600" fill="var(--alt-mesh-bg)" />` as the SVG background element.
- [ ] Add a `<defs>` block containing: blur filter (`feGaussianBlur stdDeviation="66"`, `filterUnits="userSpaceOnUse"`, region x="-1200" y="-1200" width="3800" height="3000") and fractalNoise filter (`baseFrequency="0.74"`, `octaves="3"`, `feBlend mode="overlay"`). Use distinct IDs (e.g. `home-mesh-blur`, `home-mesh-noise`).
- [ ] [P] Add the six topographic shape elements inside `<g filter="url(#home-mesh-blur)" style={{ mixBlendMode: 'var(--alt-mesh-blend)' }}>`, each with `style={{ transformBox: 'fill-box', transformOrigin: 'center', willChange: 'transform' }}` and an `animation` inline style string. Shape compositions and coordinates as specified in design.md (right-focal, x≥650 concentration). Fill values: outer loop = `var(--alt-grad-dev-s)`, inner loop = `var(--alt-grad-prev-s)`, cross-sweep = `var(--alt-grad-dev-e)`, focal core = `var(--alt-grad-prev-e)`, amber polygon = `var(--alt-grad-ship-e)`, coral ellipse = `var(--alt-grad-ship-s)`.
- [ ] [P] Add the noise overlay `<rect>` after the blurred group: `fill="#888"`, `style={{ mixBlendMode: 'overlay', opacity: 0.055 }}`, `filter="url(#home-mesh-noise)"`.
- [ ] Define `@keyframes` for each shape animation in `src/app/globals.css` under `@theme inline`: `--animate-mesh-a` through `--animate-mesh-f` with the durations (9–13 s), translate (±68 px), scale (0.93–1.09), and rotate (±8° for rotated rects).
...
- [ ] In `src/components/home/hero/index.tsx`: update `<section>` to add `overflow-hidden` (or `overflow: hidden` equivalent).
- [ ] [P] Mount `<MeshGradientBackground />` as the first child of `<section>`, before all other content. It must NOT be inside the content grid `<div>`.
- [ ] [P] Change the grid `<div>` from `lg:grid lg:grid-cols-2` to `md:grid md:grid-cols-2`. The terminal column wrapper should have `hidden md:block` so the terminal is hidden below 768 px.
- [ ] [P] Add eyebrow `<p>` above `<h1>` with `caption-mono` typography style and `hairline-strong` color. Content: "Portfolio · Taipei, Taiwan".
- [ ] [P] Update `<h1>` font size to use `clamp(48px, 5.5vw, 80px)` at ≥ md; `clamp(36px, 10vw, 56px)` below md (via responsive Tailwind class or inline style).
- [ ] Update text color tokens for dark mesh context: h1 name → `text-[var(--alt-ink)]` or `on-primary` equivalent, title → `text-[var(--alt-hairline-strong)]`, bio → `text-foreground`.
- [ ] Remove `<BackgroundAnimation />` import and usage from `src/components/home/index.tsx`. `Home` now renders only `<ProfileHero locale={lang} />`.
- [ ] Run `pnpm type-check`. Verify no type errors.
...
- [ ] Delete `src/components/home/background-animation.tsx`.
- [ ] Delete `src/components/home/background-animation.css`.
- [ ] Search for any remaining imports of `BackgroundAnimation` or `background-animation` in the codebase and remove them. Verify: `grep -r "BackgroundAnimation\|background-animation" src/` returns no results.
- [ ] Run `pnpm type-check`. Fix any broken imports.
