...
- [ ] 2.1 [P] Create `schemas/tag.ts` to implement the Tags as Array of Reference to Tag Document decision: the `tag` document type has `title` (string, required) and `slug` (slug, `source: "title"`, required) fields. Verified by: Studio loads with `tag` in the document list and enforces required fields on publish attempt.

- [ ] 2.2 [P] Create `schemas/block/card.ts`: the `card` object type has `title` (string, required) and `href` (url, required) fields; this type is registered as a custom block in Portable Text arrays. Verified by: Studio Portable Text editor shows "Insert Card" option; card block saves title and href correctly.
