...
Allows visitors to download the site owner's resume as a PDF in their preferred language directly from the About page, with support for shareable URLs that open the download dialog automatically.

## Capabilities

### pdf-storage

Resume PDFs are stored as Sanity file assets on a `siteSettings` singleton document. Up to three language variants are supported: Traditional Chinese, English, and Japanese.

### download-dialog

The About page provides a dialog (using shadcn/ui Dialog) listing download links for all available resume PDF variants. The dialog is absent when no PDFs are configured.

### shareable-url

The About page supports a `?resume=open` query parameter that opens the PDF dialog on initial page load, enabling shareable links to the resume download flow.
