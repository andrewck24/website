...
describe("Image (no coverImage)", () => {
  beforeEach(() => {
    mockGetNote.mockResolvedValue({ title: "My Note", coverImage: null });
  });

  it("calls ImageResponse with fallback JSX", async () => {
    await Image({ params: Promise.resolve({ lang: "en", slug: "my-note" }) });
  ...
  it("fallback JSX contains the note title", async () => {
    await Image({ params: Promise.resolve({ lang: "en", slug: "my-note" }) });
    const result = MockImageResponse.mock.results[0].value as {
      jsx: React.ReactElement;
    };
    const jsxString = JSON.stringify(result.jsx);
    expect(jsxString).toContain("My Note");
  });
});
...
describe("Image (with coverImage)", () => {
  ...
  it("uses Sanity image URL as img src", async () => {
    await Image({
      params: Promise.resolve({ lang: "en", slug: "cover-note" }),
    });
    const result = MockImageResponse.mock.results[0].value as {
      jsx: React.ReactElement;
    };
